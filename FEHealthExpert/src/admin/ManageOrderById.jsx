...
export default function ManageOrderByIdByAdmin() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await axios.get(`http://20.2.73.15:8173/api/Order/GetOrderById?id=${orderId}`);
                const orderList = response.data;

                //Fetch Account DTO
                const fetchAccountDTO = async (accountId) => {
                    try {
                        const response = await axios.get(`http://20.2.73.15:8173/api/Account/GetAccountById?id=${accountId}`);
