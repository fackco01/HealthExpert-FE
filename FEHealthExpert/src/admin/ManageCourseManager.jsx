...
export default function ManageCourseManager() {
    ...
    useEffect(() => {
        const fetchCourseAdmin = async () => {
            try {
                ...
                const filteredAccounts = accountList.filter((account) => account.roleId === 3);
    ...
    const handleStatusChange = async (record) => {
        try {
            ...
            if (record.isActive) {
                const response = await axios.post(`http://20.2.73.15:8173/api/Account/DeleteAccount/${record.accountId}`);
                console.log("Status updated successfully!");
                window.location.reload();
            } else {
                const response = await axios.post(`http://20.2.73.15:8173/api/Account/EnableAccount/${record.accountId}`);
                console.log("Status updated successfully!");
                window.location.reload();
            }
